...
public class ResponseController {
    ...
    @GetMapping("/get-all-foxian-names")
    public List<FoxianDTO> getAllFoxian() {
        List<Foxian> res = foxianRepository.findAll();
        return res.stream()
                .map(this::convertToDto)
                .collect(Collectors.toList());
    }

    private FoxianDTO convertToDto(Foxian foxian) {
        FoxianDTO dto = new FoxianDTO(foxian.getId(), foxian.getInternName());
        return dto;
    }
